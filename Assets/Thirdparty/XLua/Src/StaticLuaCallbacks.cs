    ...
    public partial class StaticLuaCallbacks
    {
        ...
        internal static int PrintWarning(RealStatePtr L)
        {
            try
            {
                ...
                Common.Log.Warning("LUA Warning: " + s);
                return 0;
            }
            catch (System.Exception e)
            {
        ...
        [MonoPInvokeCallback(typeof(LuaCSFunction))]
        internal static int PrintInfo(RealStatePtr L)
        {
            try
            {
                int n = LuaAPI.lua_gettop(L);
        ...
        internal static int LoadFromCustomLoaders(RealStatePtr L)
        {
            try
            {
                ...
                foreach (var loader in self.customLoaders)
                {
                    ...
                    int fileLength;
                    byte[] bytes = loader(ref real_file_path, out fileLength);
                    if (bytes != null)
                    {
                        if (LuaAPI.xluaL_loadbuffer(L, bytes, fileLength/* bytes.Length*/, "@" + real_file_path) != 0)
                        {
