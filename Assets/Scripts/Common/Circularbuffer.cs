    ...
    public class CircularBuffer: Stream
    {
        ...
        public override long Length
        {
            get
            {
                ...
                if (c < 0)
                {
                    throw new Exception($"CircularBuffer count < 0: {this.bufferQueue.Count}, {this.LastIndex}, {this.FirstIndex}");
                }
                return c;
            }
        }
		...
		/// <returns></returns>
		public async Task ReadAsync(Stream stream, int count)
	    {
            ...
            while (alreadyCopyCount < count)
            {
                ...
                {
                    await stream.WriteAsync(this.First, this.FirstIndex, n);
                    this.FirstIndex += n;
                    alreadyCopyCount += n;
                }
                else
                {
                    await stream.WriteAsync(this.First, this.FirstIndex, ChunkSize - this.FirstIndex);
		...
		public async Task WriteAsync(Stream stream, int count)
	    {
            ...
            while (alreadyCopyCount < count)
            {
                ...
                if (ChunkSize - this.LastIndex > n)
                {
                    await stream.ReadAsync(this.lastBuffer, this.LastIndex, n);
                    this.LastIndex += count - alreadyCopyCount;
                    alreadyCopyCount += n;
                }
                else
                {
                    await stream.ReadAsync(this.lastBuffer, this.LastIndex, ChunkSize - this.LastIndex);
                    alreadyCopyCount += ChunkSize - this.LastIndex;
                    this.LastIndex = ChunkSize;
                }
            }
